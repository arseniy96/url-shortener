...
func (s *GRPCServer) CreateLink(ctx context.Context, in *CreateLinkRequest) (*CreateLinkResponse, error) {
	originURL := in.Url
...
func (s *GRPCServer) CreateLinksBatch(ctx context.Context,
	in *CreateLinksBatchRequest) (*CreateLinksBatchResponse, error) {
	userSession := in.UserSession
	if len(userSession) == 0 {
		return nil, status.Errorf(codes.InvalidArgument, handlers.UserUnauthorizedErrTxt)
	}

	response := make([]*CreateLinksBatchResponseNested, 0)
	...
	for _, url := range in.Urls {
		...
		response = append(response, &CreateLinksBatchResponseNested{
	...
	return &CreateLinksBatchResponse{Urls: response}, nil
}
